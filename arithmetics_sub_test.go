	...
	tests := []testCase{
		{
			...
			TestFunc: func(v any) any {
				vs := v.(args)
				return Sub(vs.x.([]int32), vs.y.([]int32))
			},
		},
		{
			Name: "int32-align",
		...
		},
		{
			...
			TestFunc: func(v any) any {
				vs := v.(args)
				return Sub(vs.x.([]float32), vs.y.([]float32))
			},
		},
		{
			Name: "float64",
			...
			TestFunc: func(v any) any {
				vs := v.(args)
				return Sub(vs.x.([]float64), vs.y.([]float64))
			},
		},
		{
		...
		},
		{
			...
			Want: []float64{-1.1, Float64NaN(), Float64NaN(), Float64NaN()},
			TestFunc: func(v any) any {
				vs := v.(args)
				return Sub(vs.x.([]float64), vs.y)
			},
		},
	}
